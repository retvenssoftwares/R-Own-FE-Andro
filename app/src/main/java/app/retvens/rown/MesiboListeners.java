                ...
                if (!TextUtils.isEmpty(SampleAPI.getToken())) {
                    ...
                    if (TextUtils.isEmpty(u.getName())) {
                       // UIManager.launchEditProfile(mLoginContext, 0, 0, true);
                        Toast.makeText(mContext, "LAUNCH LOGIN", Toast.LENGTH_SHORT).show();
                    } else {
...
//    @Override
//    public void onProductTourCompleted(Context context) {
//        UIManager.launchLogin((Activity)context, (ILoginInterface) MesiboListeners.getInstance());
//    }
...
//        MesiboAPI.login(phone, code, mHandler);
    ...
    public static MesiboCall.IncomingListener getInstance() {
        if(null==_instance)
